...
extern void copy(register char *dest, char *src, register int n);

  /* char *dest, *src; int n; */
  /* copies exactly n bytes. */
  /* return value undefined.  Use only as procedure. */

extern void fill (register char *addr, register char ch, register int n);

  /* char *addr, ch; int n; */
  /* copies ch into n consecutive bytes. */
  /* return value undefined.  Use only as procedure. */

extern char to_upper_if_lower (char ch);

  /* char ch;  Returns possibly upper-cased value. */

extern char to_lower_if_upper(char ch);
...
extern int nbuffconcat (char *buffer, int n, char *s1, char *s2, char *s3, char *s4, char *s5, char *s6);
...
extern char *strncfind( register char *s1, char *s2, Bool fast);
...
extern char *strsearch(register char *s1, char *s2, register int s1len, int s2len);
...
extern char *strncsearch(register char *s1, char *s2, register int s1len, int s2len);
...
extern Bool all_digits (char *astring);
extern Bool all_whitespace (char *astring);
extern Bool all_uppercase(char *astring);
extern Bool all_lowercase(char *astring);
extern Bool all_alphabetic(char *astring);
extern Bool all_alphanumeric(char *astring);
extern Bool all_ascii(char *astring);


extern int str_to_pos_int (char *astring, int low,int high);
...
extern int sreverse (char *buffer, char *astring);
...
extern int ip_string_trim (char *oldstring, char *trimchars, Bool pretrim, Bool posttrim);
