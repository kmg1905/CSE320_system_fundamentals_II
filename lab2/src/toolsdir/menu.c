...
//static char *line = (char *) malloc(MAX_MENU_RESPONSE_LENGTH);

int menu_match (int *ptr_rval, char **ptr_ur, char *prompt, int casetest, int isub, int r_no_match,
  int r_ambiguous, int n_options, ... )
{

  char sline[MAX_MENU_RESPONSE_LENGTH];
  //size_t buf_length = MAX_MENU_RESPONSE_LENGTH;
  //char *option;
  ...
  if (n_options > MAX_MENU_OPTIONS)
    return (MENU_ERROR);
  for (j = 0; j < MAX_MENU_RESPONSE_LENGTH; j++)
    line[j] = ' ';
  ...
  printf("%s",prompt);
  fflush(stdout);
  switch (rval = getline1(stdin, line, MAX_MENU_RESPONSE_LENGTH)) {
    case AT_EOF :
      return(MENU_EOF);
      break;
    case TOO_MANY_CHARS :
...

{
  ...
  if (!allow_help)
  {
     rval = menu_match (&menuval, &response, prompt,
          0,1,0,0,2,
          "Yes", MENU_YES,
          "No", MENU_NO
       );
...
extern int menu_data_help_or_abort (char *prompt, char *abortstring, char **ptr_response)
...

{
  ...
  {
    char ch[] = "\0";
    char ch2[] = "Please enter a non-negative number...\n";
     nbuffconcat(errprompt,1,ch2, ch,ch,ch,ch,ch);
  }
  else
