...
void copy (register char *dest, char *src, register int n)
...
void fill (register char *addr, register char ch, register int n)
...
char to_upper_if_lower (char ch)

  { return(islower(ch) ? toupper(ch) : ch); }


char to_lower_if_upper (char ch)

  { return(isupper(ch) ? tolower(ch) : ch); }


void buffconcat (char *buffer, char *s1, char *s2)
...
int slcompare (char *s1, int l1, char *s2, int l2)
...
int slge_compare (char *s1, int l1, char *s2, int l2)
...
char *strfind(register char *s1, char *s2, Bool fast)
{
...
char *strncfind( register char *s1, char *s2, Bool fast)
{
  register int len1,len2;
  len2 = strlen(s2);
  if (fast) {
...
char *strsearch(register char *s1, char *s2, register int s1len, int s2len)
...
char *strncsearch(register char *s1, char *s2, register int s1len, int s2len)
...
int remove_excess_blanks (char *newstring, char *oldstring)
...
int ip_string_trim (char *oldstring, char *trimchars, Bool pretrim, Bool posttrim)
...
int string_trim (char *newstring, char *oldstring, char *trimchars, Bool pretrim, Bool posttrim)
{
...
Bool all_whitespace (char *astring)
...
Bool all_uppercase(char *astring)
{
  register char ch;
  while ((ch = *astring++) != '\0') {
...
Bool all_lowercase(char *astring)
{
  register char ch;
  while ((ch = *astring++) != '\0') {
...
Bool all_alphabetic(char *astring)
{
  register char ch;
  while ((ch = *astring++) != '\0') {
...
Bool all_alphanumeric(char *astring)
{
  register char ch;
  while ((ch = *astring++) != '\0') {
...
int str_to_pos_int (char *astring, int low,int high)
...
int sreverse (char *buffer, char *astring)
{
...

{
  ...
  char ch[] = "/";
  char ch2[] = "\0";
  nbuffconcat(pathbuffer,4,dir,ch,filename,extension,ch2,ch2);
...
int read_yes_or_no (FILE *iport, FILE *oport, char *prompt, char *helpstring, char *quitstring)
...

{
  ...
  while (T) {
    ...
    switch (len = getline1(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int getline1 (FILE *iport, char *buffer, int buflen)
...
int getlines (FILE *fp, int n, char ***ptr_lines, char *linebuf, int maxlinelen)
...

{
  int len;
  char *line;
  if (0 > (len = getline1(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
...
int getfile (char *filename, char ***ptr_lines, char *linebuf, int maxlinelen)
...
int ngetlines (FILE *fp, int n, char ***ptr_lines, char *linebuf, int maxlinelen)
...
int ngetfile (int n, char *filename, char ***ptr_lines, char *linebuf, int maxlinelen)
...
extern int read_file_into_buffer (char *filename, char ***ptr_lines, int maxlines, char *buffer,
  int buflen, char *linebuffer, int linebuflen)
...

{
  ...
  while (T) {
    ...
    if (0 > (len = getline1(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
...
Bool check_string (char *s, long minlen, long maxlen)
{
  long len;
  if (s == 0) return(F);
  len = strlen(s);
