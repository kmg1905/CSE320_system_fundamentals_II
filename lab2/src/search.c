...

{
  ...
  while (rlist != 0) {
    ...
    if (strncsearch(name,search_string,strlen(name), sslen) ||
        strncsearch(company,search_string,strlen(company), sslen)) {
       set_matched(rlist);
       count++;
    }
  }
  ...
  if (field_index == OTHER) {
     for (j = 0; j < get_n_others(entry); j++) {
         ...
         *field++ = ':';
         ...
         if (strncsearch(field,search_string,strlen(field), sslen)) {
            set_matched(rlink);
            return(1);
         }
     }
